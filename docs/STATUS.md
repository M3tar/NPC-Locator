...
1. 整理 `README.md`、`README.zh-CN.md`、`CHANGELOG.md` 和已知限制。
2. 新增阶段 5 最终验收清单，聚焦双方同版、单边缺少 Mod、主机关闭共享、断线和版本/协议边界。
3. 补测 UI 缩放、手柄、雨天、节日/事件、无标准日程和公告栏任务过期。
4. Windows 生成 Release DLL 与可直接解压到 `Mods` 的 0.1.0 二进制包，主机和农场助手使用同一包做最终冒烟。
5. 检查双方 SMAPI 日志无本 Mod 未处理错误后，标记 0.1.0 私人内测候选版。
