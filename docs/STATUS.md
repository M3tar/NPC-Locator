...
> 最后更新：2026-07-31 18:41 CST

## 当前阶段

阶段 2：正式查询 UI（单人和非主机联机核心查询及羊皮纸界面已通过实际验证，进入功能收尾）。阶段 1 的主机—客户端查询闭环已通过。
...
1. 修复 `SebastianRoom` 一类内部地点名的本地化回退，并验证联机主机的 F3 本地查询。
2. 接入 GMCM，补齐手动刷新、追踪按钮、NPC 列表过滤和基础手柄操作。
3. 补测中文搜索、列表/日程滚轮、Esc/F3/关闭按钮和另一种 UI 缩放或分辨率。
...
- 阶段 2 尚未完成主机 F3 查询、地点名回退、不同 UI 缩放和完整输入验证。
