...
阶段 4：阶段 3 的地点本地化、GMCM、主机/农场助手查询与单 NPC 追踪已通过用户实际测试。标准物品交付任务的本地识别、去重提示与任务关联追踪已编码，等待 Windows 编译和实机验证。
...
1. 在 Windows 1.6.15 / SMAPI 4.5.2 编译阶段 4 源码包。
2. 用现有“作物研究”剧情任务完成提示、物品持有数、追踪及结束清理测试。
3. 接受一个公告栏随机物品交付任务，验证剩余天数和过期清理。
4. 补测 UI 缩放、手柄和远程共享关闭等边界。
5. 阶段 4 通过后进入阶段 5 联机验收、说明与正式打包。
...
- 阶段 4 任务识别新代码尚未在 Windows 游戏程序集上编译；当前 macOS 开发机没有游戏和 SMAPI 程序集。
